...
class Add extends React.Component {
    constructor(){
      ...
      handleUploadPhoto = async() => {
        ...
        fetch("http://192.168.43.209:8500/api/children/add/2",{
            method:"POST",
      ...
      render() {
        ...
        return (
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center'}}>
            <Text style={{flex:1,color:'dodgerblue', fontSize:40, textAlign:'center',marginTop:30}}>Add Missing Child</Text>
            {/* {this.state.photo && (
              <View>
                <Image
...
class Main extends React.Component {
    ...
    componentDidMount() {
        Axios.get('http://192.168.43.209:8500/api/children/2')
        .then(res=> {
    ...
    render() {
        ...
        return(
            <View style= {{flex:1}}>
              {/* <Button title = 'add items' onPress = {()=> this.props.navigation.navigate('Add')}/> */}
              <Text style={{flex:1,color:'dodgerblue', fontSize:50, textAlign:'center',marginTop:50}}>Public Portal</Text>
            ...
            <TouchableOpacity onPress={()=> this.props.navigation.navigate('Add')} style={{flex:0, backgroundColor :"dodgerblue", borderRadius:5,padding:15,paddingHorizontal:20, alignSelf:'flex-end', margin:20}}>
